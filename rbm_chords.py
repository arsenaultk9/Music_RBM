...
with tf.Session() as sess:
    ...
    for epoch in tqdm(list(range(num_epochs))):
        for song in songs:
            ...
            song = np.array(song)
            sampling_size = int(np.floor(song.shape[0]/num_timesteps)*num_timesteps)
            song = song[:sampling_size]
            song = np.reshape(
                song, [int(song.shape[0]/num_timesteps), int(song.shape[1]*num_timesteps)])
